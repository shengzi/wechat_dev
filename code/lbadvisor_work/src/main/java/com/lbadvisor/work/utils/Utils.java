...
public class Utils {
     ...
     * @return
     */
    public static String getUrl(String url, int w, int h, String f) {
        return url.replace("{w}", String.valueOf(w)).replace("{h}", String.valueOf(h)).replace("{f}", f);
    }
     ...
     * @return 统计粒度 "HOURLY, DAILY, WEEKLY, MONTHLY"
     */
    public static List<Map<String, Object>> getGranularityList(DateTime startTime, DateTime endTime, String selectedGranularity, String reportType) {
        Map<String, String> notHourlyMap = new HashMap<>();
        notHourlyMap.put("age", "age");
        notHourlyMap.put("gender", "gender");
        notHourlyMap.put("location", "location");
        Period p = new Period(startTime, endTime, PeriodType.days());
        int day = p.getDays();
        List<Map<String, Object>> granularityList = new LinkedList<>();
        Set<String> granularitySet = new LinkedHashSet<>();
        if (day <= 7) {
            if (!notHourlyMap.containsKey(reportType)) {
                granularitySet.add("时");
            }
            granularitySet.add("日");
        } else if (day > 7 && day < 14) {
            granularitySet.add("日");
        } else if (day >= 14 && day < 60) {
            granularitySet.add("日");
            granularitySet.add("周");
        } else if (day >= 60 && day < 90) {
            granularitySet.add("日");
            granularitySet.add("周");
            granularitySet.add("月");
        } else if (day >= 90) {
            granularitySet.add("周");
            granularitySet.add("月");
        }
        Iterator<String> it = granularitySet.iterator();
        while (it.hasNext()) {
            String g = it.next();
            Map<String, Object> item = new HashMap<>();
            item.put("granularityOpt", g);
            if (getGranularityDisplayName(selectedGranularity).equals(g)) {
                item.put("selected", true);
            } else {
                item.put("selected", false);
            }

            granularityList.add(item);
        }
     ...
     * @return 统计粒度 "HOURLY, DAILY, WEEKLY, MONTHLY"
     */
    public static List<Map<String, Object>> getGranularityList(DateTime startTime, DateTime endTime, String selectedGranularity) {
        Period p = new Period(startTime, endTime, PeriodType.days());
        int day = p.getDays();
        List<Map<String, Object>> granularityList = new LinkedList<>();
        Set<String> granularitySet = new LinkedHashSet<>();
    ...
    public static Map<String, Object> setParams(Map<String, Object> params) {
        for (Map.Entry<String, Object> map : params.entrySet()) {
            try {
                if (map.getValue().toString().length() <= 2) {
                    continue;
                }
                String[] values = map.getValue().toString().split("\\|");
                if (values.length > 1) {
                    StringBuilder paramValue = new StringBuilder();
                    for (int i = 0; i < values.length; i++) {
                        String value = values[i].substring(2);
                        String mathch = values[i].substring(0, 2);;
                        if (mathch.equals("gt")) {
                            if (i == 0) {
                                paramValue.append(">".concat(value));
                            } else {
                                paramValue.append(" AND "+map.getKey()+">".concat(value));
                            }
                        } else if (mathch.equals("lt")) {
                            if (i == 0) {
                                paramValue.append("<".concat(value));
                            } else {
                                paramValue.append(" AND "+map.getKey()+"<".concat(value));
                            }
                        } else if (mathch.equals("eq")) {
                            if (i == 0) {
                                paramValue.append("=".concat(value));
                            } else {
                                paramValue.append(" AND "+map.getKey()+"=".concat(value));
                            }
                        } else if (mathch.equals("ge")) {
                            if (i == 0) {
                                paramValue.append(">=".concat(value));
                            } else {
                                paramValue.append(" AND "+map.getKey()+">=".concat(value));
                            }
                        } else if (mathch.equals("le")) {
                            if (i == 0) {
                                paramValue.append("<=".concat(value));
                            } else {
                                paramValue.append(" AND "+map.getKey()+"<=".concat(value));
                    ...
                    params.put(map.getKey(), paramValue.toString());
                } else {
                    String value = map.getValue().toString().substring(2);
                    String mathch = map.getValue().toString().substring(0, 2);
                    if (mathch.equals("gt")) {
                        params.put(map.getKey(), ">".concat(value));
                    } else if (mathch.equals("lt")) {
                        params.put(map.getKey(), "<".concat(value));
                    } else if (mathch.equals("eq")) {
                        params.put(map.getKey(), "=".concat(value));
                ...
                log.error("set params , exception : ", e);
            }
        }
        return params;
    }
    public static void main(String[] args) {
        System.out.println(Utils.getUUIDStr());
    }
}
